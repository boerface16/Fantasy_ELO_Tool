      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-border-line bg-bg-elevated/40">
            ...
            {!isDelta && <th className="px-4 py-3 text-right text-xs font-semibold text-text-secondary uppercase">Top %</th>}
          </tr>
        </thead>
        <tbody>
          {isLoading ? (
...
function TalentLeaderboardRow({
  ...
  const tier = getEloTier(player.season_elo ?? 1500);
    ...
    >
      ...
      {isDelta ? (
        ...
        <td className={`px-4 py-3 text-sm font-bold text-right ${tierColor}`}>
          {Math.round(player.season_elo ?? 1500)}
        </td>
      )}
